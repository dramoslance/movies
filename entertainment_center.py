...
actors = ["Tom Hanks", "Jennifer Lawrence", "Zoe Saldaña", "Shailene Woodley", "Owen Wilson", "Rob Estes" ]
...
for i in range(len(titles)):
    ...
    current_movie.set_synopsis_url(informations[i])
    current_movie.set_story_line((story_lines[i][:500]) if len(story_lines[i]) > 500 else story_lines[i])
    current_movie.set_price(prices[i])
    current_movie.set_poster_image(posters[i])
    current_movie.set_url_trailer(trailers[i])
    current_movie.set_publishment_date(dates[i])

    current_actor = media.Actor(actors[i], actors_genders[i], actors_nationalities[i], actors_ages[i])            
